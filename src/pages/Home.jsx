...
export default function Home() {
  ...
  return (
    <div className="screen">
      ...
      {/* TOPBAR */}
      <div className="animate-fade-in" style={{ background: 'var(--primary)', padding: '16px 16px 13px', position: 'sticky', top: 0, zIndex: 10, boxShadow: '0 4px 16px rgba(0,0,0,0.12)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <div>
            <div style={{ fontFamily: "'Fredoka One', cursive", fontSize: '21px', color: '#F0E8D8' }}>Mercadinho da Carmen</div>
            <div style={{ fontSize: 11, color: 'var(--text-on-dark-muted)', fontWeight: 800, letterSpacing: '.1em', textTransform: 'uppercase', marginTop: 1 }}>📍 Vila São José · Taubaté</div>
          </div>
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={() => nav('/perfil')} className="btn-press tap-target" style={{ width: 40, height: 40, borderRadius: '50%', background: 'rgba(255,255,255,.15)', border: 'none', color: '#fff', fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>👤</button>
        ...
        <div onClick={() => nav('/categorias')} className="btn-press" style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(255,255,255,.13)', borderRadius: 14, padding: '13px 16px', cursor: 'pointer' }}>
          <span style={{ fontSize: 16 }}>🔍</span>
          <span style={{ color: 'rgba(255,255,255,0.7)', fontSize: 15, fontWeight: 700 }}>O que você precisa hoje?</span>
        </div>
      </div>

      {/* HERO DINÂMICO POR HORÁRIO */}
      <div className="animate-fade-in stagger-1" style={{ background: 'var(--primary-dark)', padding: '28px 16px 0', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', minHeight: 130, position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 16, background: 'var(--bg)', borderRadius: '16px 16px 0 0' }} />
        <div style={{ zIndex: 1, paddingBottom: 28 }}>
          <div style={{ fontFamily: "'Fredoka One', cursive", fontSize: '24px', color: '#fff', lineHeight: 1.15, marginBottom: 8 }}>
            {tc.hero.titulo}
            <br />
            <span style={{ color: 'var(--accent)', fontSize: 19 }}>{tc.hero.sub}</span>
          </div>
          <div style={{ background: tc.hero.cor, color: '#fff', fontSize: 11, fontWeight: 900, padding: '5px 12px', borderRadius: 99, display: 'inline-block', marginTop: 8, letterSpacing: '.06em' }}>
            {tc.hero.destaque}
          </div>
        </div>
        <div className="animate-bounce" style={{ fontSize: 68, zIndex: 1, paddingBottom: 16 }}>
          {isWeekend ? '🎉' : isEvening ? '🍷' : '🛵'}
        </div>
      </div>
      ...
      {isWeekend && (
        <div className="animate-slide-up" style={{ margin: '16px 16px 0' }}>
          <div
            onClick={() => nav('/adega')}
            className="btn-press shimmer-overlay card-pressable"
            style={{
          ...
          >
            ...
            <div>
              ...
              <div style={{ fontSize: 13, color: 'rgba(243,237,221,0.9)', fontWeight: 700, marginTop: 4 }}>
                A partir de R$ 22,90 · Entrega 20 min
              </div>
            </div>
            <div style={{ fontFamily: "'Fredoka One', cursive", fontSize: 24, color: '#fff', flexShrink: 0 }}>→</div>
          </div>
        </div>
      )}
      ...
      <div className="animate-slide-up stagger-2" style={{ padding: isWeekend ? '12px 16px 0' : '20px 16px 0' }}>
        <div
          onClick={() => nav('/adega')}
          className="btn-press card-pressable"
          style={{
        ...
        >
          ...
          <div style={{ width: 54, height: 54, borderRadius: 16, background: 'rgba(200,146,42,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, flexShrink: 0, border: '1px solid rgba(200,146,42,0.3)' }}>
            🍷
          </div>
          <div style={{ flex: 1, zIndex: 1 }}>
            <div style={{ fontFamily: "'Fredoka One', cursive", fontSize: 17, color: '#F5C842' }}>Adega da Carmen</div>
            <div style={{ fontSize: 13, color: 'var(--text-brand-gold)', fontWeight: 800, marginTop: 2 }}>
              Cervejas, vinhos, destilados · Desconto até 15%
            </div>
          </div>
          <div style={{ fontSize: 22, color: '#C8922A', flexShrink: 0, zIndex: 1 }}>›</div>
        </div>
      </div>

      {/* CATEGORIAS */}
      <div className="animate-slide-up stagger-2" style={{ padding: '18px 16px 0' }}>
        <div style={{ display: 'flex', gap: 14, overflowX: 'auto', scrollbarWidth: 'none', paddingBottom: 8, scrollSnapType: 'x mandatory' }}>
          {CATS_ORDER.map(cid => {
            const c = CATEGORIAS[cid]
            return (
              <div key={cid} onClick={() => nav('/categoria/' + cid)} className="btn-press card-pressable" style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, cursor: 'pointer', scrollSnapAlign: 'start' }}>
                <div style={{ width: 72, height: 72, borderRadius: '22px', background: c.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 34, border: '2px solid ' + c.cor + '22', boxShadow: '0 4px 12px rgba(0,0,0,0.06)' }}>
                  {c.emoji}
                </div>
                <span style={{ fontSize: 13, fontWeight: 800, color: 'var(--text)' }}>{c.nome}</span>
              </div>
            )
          })}
        </div>
      ...
      <div className="animate-slide-up stagger-3" style={{ padding: '20px 16px 0' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <span style={{ fontFamily: "'Fredoka One', cursive", fontSize: '20px', color: 'var(--text)' }}>⭐ Seus favoritos</span>
          <span onClick={() => nav('/categorias')} className="btn-press" style={{ fontSize: 14, color: 'var(--accent)', fontWeight: 800, cursor: 'pointer' }}>ver tudo</span>
        </div>
        <div style={{ display: 'flex', gap: 12, overflowX: 'auto', scrollbarWidth: 'none', padding: '4px 0 16px', margin: '0 -16px', paddingLeft: 16, paddingRight: 16, scrollSnapType: 'x mandatory' }}>
          {destaques.map((p, i) => (
            <div key={p.id} style={{ flexShrink: 0, width: 155, scrollSnapAlign: 'start' }}>
      ...
      <div className="animate-slide-up stagger-4" style={{ padding: '4px 16px 0' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <span style={{ fontFamily: "'Fredoka One', cursive", fontSize: '20px', color: 'var(--text)' }}>🎁 Combos da Carmen</span>
        </div>
      ...
      <div className="animate-slide-up" style={{ padding: '28px 16px 0', animationDelay: '0.4s' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <span style={{ fontFamily: "'Fredoka One', cursive", fontSize: '20px', color: 'var(--text)' }}>{tc.sectionLabel}</span>
          {isEvening
            ? <span onClick={() => nav('/adega')} className="btn-press" style={{ fontSize: 14, color: '#C8922A', fontWeight: 800, cursor: 'pointer' }}>ver adega</span>
            : <span onClick={() => nav('/categorias')} className="btn-press" style={{ fontSize: 14, color: 'var(--accent)', fontWeight: 800, cursor: 'pointer' }}>ver tudo</span>
          }
        </div>
      ...
      <div className="animate-slide-up" style={{ padding: '28px 16px 0', animationDelay: '0.5s' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <span style={{ fontFamily: "'Fredoka One', cursive", fontSize: '20px', color: 'var(--text)' }}>⚡ Ofertas Relâmpago</span>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {FLASH.map((f, i) => (
            <div key={i} className="btn-press card-pressable shimmer-overlay" style={{
              background: f.cor, borderRadius: 'var(--radius)', padding: '18px',
              minHeight: 110, position: 'relative', overflow: 'hidden', cursor: 'pointer',
              boxShadow: `0 8px 20px ${f.cor}44`
            }}>
              <div style={{ fontSize: 11, fontWeight: 900, color: 'rgba(243,237,221,0.92)', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '.06em' }}>{f.label}</div>
              <div style={{ fontFamily: "'Fredoka One', cursive", fontSize: '22px', color: '#fff' }}>{f.sub}</div>
